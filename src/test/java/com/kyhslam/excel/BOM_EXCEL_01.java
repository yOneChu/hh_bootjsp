...
public class BOM_EXCEL_01 {
    ...
    public static void main(String[] args) {
        ...
        String fileName = "d:\\excel\\BOM원복리스트_207대_260413_검토_ver2.xlsx";
    ...
    public static void writeBOM(Workbook workbook, ArrayList<String> hogiList) {
        ...
        try {
            ...
            for(int i=0; i < hogiList.size(); i++){
                ...
                ArrayList<ProductDto> bomList = ProductCommonUtil.findProductInfo(hogi);


                for(int j=0; j < bomList.size(); j++){

                    ProductDto bom = bomList.get(j);

                    String partNo = bom.getPartNo();
                    String partName = bom.getPartName();
                    String blockOpt = bom.getBlockopt();
                    String nation = bom.getNation();
                    String blockNo = bom.getBlockNo();
                    ...
                    row.createCell(4).setCellValue(blockOpt);
                    row.createCell(5).setCellValue(nation);
